 ...
 *   - Single buffer used by both network and ISR
 *   - Producer (network_task): writes via frame_buffer_write()
 *   - Consumer (GPTimer ISR): reads via frame_buffer_read_isr()
 * 
 * No mutex needed! Uses memory barriers for cross-core synchronization.
 * ISR functions use lighter memory ordering for speed.
 */
...
static laser_point_t buffer[FRAME_BUFFER_SIZE] __attribute__((aligned(4)));
...
// These use simpler memory ordering since ISR runs very frequently
//============================================================================

// Get raw buffer pointer for direct ISR access
laser_point_t* IRAM_ATTR frame_buffer_get_buffer(void)
{
    return buffer;
}

// Get head index (ISR-safe, non-blocking)
size_t IRAM_ATTR frame_buffer_get_head(void)
{
    return atomic_load_explicit(&head, memory_order_acquire);
}

// Get tail index (ISR-safe, non-blocking)
size_t IRAM_ATTR frame_buffer_get_tail(void)
{
    return atomic_load_explicit(&tail, memory_order_relaxed);
}

// Advance tail by 1 after reading a point in ISR
void IRAM_ATTR frame_buffer_advance_tail(void)
{
    size_t t = atomic_load_explicit(&tail, memory_order_relaxed);
    t = (t + 1) % FRAME_BUFFER_SIZE;
    atomic_store_explicit(&tail, t, memory_order_release);
...
// Check if buffer has data (ISR-safe)
...
// Get free space in buffer (for network side to know how much to accept)
