...
static void fill_status(etherdream_status_t* status) {
    ...
    // CRITICAL: Client uses "cap = 1700 - buffer_fullness" to decide how much to send.
    // Original EtherDream: 1800 point buffer, reports actual fullness 0-1800
    // Our buffer: 8192 points (FRAME_BUFFER_SIZE)
    //
    // STRATEGY: Scale our 0-8192 range to 0-1600 range
    // This way:
    //   - When buffer empty (0): report 0, client sends cap=1700 (lots)
    //   - When buffer half (4096): report 800, client sends cap=900 (moderate)
    //   - When buffer full (8192): report 1600, client sends cap=100 (little)
    //
    // This keeps throughput proportional to available space!
    size_t level = frame_buffer_level();
    
    // Scale: fullness = level * 1600 / 8192 = level * 25 / 128
    // This maps 0-8192 to 0-1600, ensuring cap always >= 100
    uint16_t fullness = (uint16_t)((level * 25) / 128);
    if (fullness > 1600) fullness = 1600;
...
void etherdream_server_loop(void) {
    ...
    if (now - s_meter_last_time >= RATE_METER_INTERVAL_MS) {
        ...
        if (elapsed_ms > 0 && s_points_received > 0) {
            ...
            // Log actual receive rate to diagnose network throughput
            ESP_LOGI(TAG, "TCP RX: %lu pts in %lums = %lu pps (target %lu)", 
                     (unsigned long)s_points_received, 
...
static void accept_client(void) {
    // Use select() with SHORT timeout - we're in a fast loop
    fd_set read_fds;
    FD_ZERO(&read_fds);
    FD_SET(s_listen_socket, &read_fds);
    
    struct timeval tv = { .tv_sec = 0, .tv_usec = 1000 }; // 1ms timeout - fast polling
...
static void handle_client(void) {
    // FAST PATH: When streaming, process as much data as possible without waiting
    // This is critical for high point rates - we need to ACK quickly!
    
    // Try multiple receives per call to maximize throughput
    for (int i = 0; i < 10; i++) {
        ssize_t len = recv(s_client_socket, s_rx_buf, sizeof(s_rx_buf), MSG_DONTWAIT);
